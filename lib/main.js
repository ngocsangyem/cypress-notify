...
utils$x.toCommandProperties = utils$x.toCommandValue = void 0;
/**
...
utils$x.toCommandValue = toCommandValue;
/**
 *
...
utils$x.toCommandProperties = toCommandProperties;
...
const os$3 = __importStar$2(require$$0$1);
...
Object.defineProperty(stringify$6, "__esModule", {
  value: true
});
stringify$6.default = void 0;
...
function stringify$5(arr, offset = 0) {
...
var _default$a = stringify$5;
stringify$6.default = _default$a;
...
var _stringify$2 = _interopRequireDefault$8(stringify$6);
...
var _stringify$1 = _interopRequireDefault$6(stringify$6);

var _parse = _interopRequireDefault$6(parse$5);
...
var _stringify = _interopRequireDefault$3(stringify$6);
...
(function (exports) {
	...
	var _stringify = _interopRequireDefault(stringify$6);
...
// We use any as a valid input type
/* eslint-disable @typescript-eslint/no-explicit-any */
const fs$a = __importStar$1(require$$0$3);
const os$2 = __importStar$1(require$$0$1);
const uuid_1 = dist$5;
...
function issueFileCommand(command, message) {
    ...
    fs$a.appendFileSync(filePath, `${utils_1.toCommandValue(message)}${os$2.EOL}`, {
        encoding: 'utf8'
    });
}
...
util$5.inherits(TunnelingAgent, events.EventEmitter);
...
var isStream$1 = isStream$2.exports = function (stream) {
...
const EventEmitter = eventemitter3$1.exports;
...
var buildURL$1 = function buildURL(url, params, paramsSerializer) {
  ...
  } else if (utils$v.isURLSearchParams(params)) {
    serializedParams = params.toString();
  } else {
    var parts = [];

    utils$v.forEach(params, function serialize(val, key) {
      if (val === null || typeof val === 'undefined') {
        return;
      }

      if (utils$v.isArray(val)) {
        key = key + '[]';
      } else {
        val = [val];
      }

      utils$v.forEach(val, function parseValue(v) {
        if (utils$v.isDate(v)) {
          v = v.toISOString();
        } else if (utils$v.isObject(v)) {
          v = JSON.stringify(v);
        }
...
InterceptorManager$1.prototype.forEach = function forEach(fn) {
  utils$u.forEach(this.handlers, function forEachHandler(h) {
    if (h !== null) {
      fn(h);
    }
  });
...
var normalizeHeaderName$1 = function normalizeHeaderName(headers, normalizedName) {
  utils$t.forEach(headers, function processHeader(value, name) {
...
utils$s.inherits(AxiosError$2, Error, {
...
AxiosError$2.from = function(error, code, config, request, response, customProps) {
  ...
  utils$s.toFlatObject(error, axiosError, function filter(obj) {
...
function toFormData$1(obj, formData) {
  ...
  function convertValue(value) {
    ...
    if (utils$r.isDate(value)) {
      return value.toISOString();
    }

    if (utils$r.isArrayBuffer(value) || utils$r.isTypedArray(value)) {
  ...
  function build(data, parentKey) {
    if (utils$r.isPlainObject(data) || utils$r.isArray(data)) {
      ...
      utils$r.forEach(data, function each(value, key) {
        if (utils$r.isUndefined(value)) return;
        var fullKey = parentKey ? parentKey + '.' + key : key;
        var arr;

        if (value && !parentKey && typeof value === 'object') {
          if (utils$r.endsWith(key, '{}')) {
            // eslint-disable-next-line no-param-reassign
            value = JSON.stringify(value);
          } else if (utils$r.endsWith(key, '[]') && (arr = utils$r.toArray(value))) {
            // eslint-disable-next-line func-names
            arr.forEach(function(el) {
              !utils$r.isUndefined(el) && formData.append(fullKey, convertValue(el));
            });
            return;
          }
        }
...
function runJob(iterator, key, item, callback)
{
  ...
  {
    aborter = iterator(item, async$9(callback));
  }
  // otherwise go with full three arguments
  else
  {
    aborter = iterator(item, key, async$9(callback));
  }
...
function terminator$2(callback)
{
  ...
  async$8(callback)(null, this.results);
}
...
function requirePopulate () {
	if (hasRequiredPopulate) return populate$2;
	hasRequiredPopulate = 1;
	// populates missing values
	populate$2 = function(dst, src) {
...
function requireForm_data () {
	if (hasRequiredForm_data) return form_data$1;
...
function setContentTypeIfUnset(headers, value) {
  if (!utils$q.isUndefined(headers) && utils$q.isUndefined(headers['Content-Type'])) {
...
var defaults$3 = {
  ...
  transformRequest: [function transformRequest(data, headers) {
    ...
    if (utils$q.isFormData(data) ||
      utils$q.isArrayBuffer(data) ||
    ...
    var isObjectPayload = utils$q.isObject(data);
    var contentType = headers && headers['Content-Type'];

    var isFileList;

    if ((isFileList = utils$q.isFileList(data)) || (isObjectPayload && contentType === 'multipart/form-data')) {
  ...
  transformResponse: [function transformResponse(data) {
    ...
    if (strictJSONParsing || (forcedJSONParsing && utils$q.isString(data) && data.length)) {
      try {
...
utils$q.forEach(['post', 'put', 'patch'], function forEachMethodWithData(method) {
  defaults$3.headers[method] = utils$q.merge(DEFAULT_CONTENT_TYPE);
});
...
var mergeConfig$2 = function mergeConfig(config1, config2) {
  ...
  function getMergedValue(target, source) {
    if (utils$n.isPlainObject(target) && utils$n.isPlainObject(source)) {
      return utils$n.merge(target, source);
  ...
  function mergeDeepProperties(prop) {
    if (!utils$n.isUndefined(config2[prop])) {
      return getMergedValue(config1[prop], config2[prop]);
    } else if (!utils$n.isUndefined(config1[prop])) {
  ...
  utils$n.forEach(Object.keys(config1).concat(Object.keys(config2)), function computeConfigValue(prop) {
...
utils$m.forEach(['delete', 'get', 'head', 'options'], function forEachMethodNoData(method) {
  /*eslint func-names:0*/
...
function createInstance(defaultConfig) {
  ...
  utils$l.extend(instance, Axios.prototype, context);

  // Copy context to instance
  utils$l.extend(instance, context);
...
FormData$1.prototype._lengthRetriever = function(value, callback) {
  ...
  if (value.hasOwnProperty('fd')) {
    ...
    // not that fast snoopy
      ...
      // still need to fetch file size from fs
      fs$9.stat(value.path, function(err, stat) {
...
FormData$1.prototype._getContentDisposition = function(value, options) {
  ...
  if (typeof options.filepath === 'string') {
    // custom filepath for relative paths
    filename = path$b.normalize(options.filepath).replace(/\\/g, '/');
  } else if (options.filename || value.name || value.path) {
    // custom filename take precedence
    // formidable and the browser add a name property
    // fs- and request- streams have path property
    filename = path$b.basename(options.filename || value.name || value.path);
  } else if (value.readable && value.hasOwnProperty('httpVersion')) {
    // or try http response
    filename = path$b.basename(value.client._httpMessage.path || '');
  }
...
Object.defineProperty(messageMetadata, "__esModule", { value: true });
...
Object.defineProperty(blocks$2, "__esModule", { value: true });
...
(function (exports) {
	...
	Object.defineProperty(exports, "__esModule", { value: true });
	__exportStar(calls, exports);
	__exportStar(dialog, exports);
	__exportStar(messageMetadata, exports);
	__exportStar(messageAttachments, exports);
	__exportStar(views, exports);
	__exportStar(blocks$2, exports);
	__exportStar(compositionObjects, exports);
	__exportStar(blockElements, exports);
	__exportStar(extensions, exports);
...
function requireMethods () {
	...
	(function (exports) {
		...
		const eventemitter3_1 = eventemitter3.exports;
		const WebClient_1 = requireWebClient();
...
var name = "@slack/web-api";
var version = "6.9.0";
var description = "Official library for using the Slack Platform's Web API";
var author = "Slack Technologies, LLC";
var license = "MIT";
var keywords = [
	"slack",
	"web-api",
...
var repository = "slackapi/node-slack-sdk";
var homepage = "https://slack.dev/node-slack-sdk/web-api";
var publishConfig = {
	access: "public"
};
var bugs = {
...
instrument.getUserAgent = instrument.addAppMetadata = void 0;
const os$1 = __importStar(require$$0$1);
...
helpers$1.default = delay;

var fileUpload = {};

Object.defineProperty(fileUpload, "__esModule", { value: true });
fileUpload.buildInvalidFilesUploadParamError = fileUpload.buildMultipleChannelsErrorMsg = fileUpload.buildChannelsWarning = fileUpload.buildFilesUploadMissingMessage = fileUpload.buildGeneralFilesUploadWarning = fileUpload.buildLegacyMethodWarning = fileUpload.buildMissingExtensionWarning = fileUpload.buildMissingFileNameWarning = fileUpload.buildLegacyFileTypeWarning = fileUpload.buildFileSizeErrorMsg = fileUpload.buildMissingFileIdError = fileUpload.warnIfLegacyFileType = fileUpload.warnIfMissingOrInvalidFileNameAndDefault = fileUpload.errorIfInvalidOrMissingFileData = fileUpload.errorIfChannelsCsv = fileUpload.warnIfChannels = fileUpload.warnIfNotUsingFilesUploadV2 = fileUpload.getAllFileUploadsToComplete = fileUpload.getFileDataAsStream = fileUpload.getFileDataLength = fileUpload.getFileData = fileUpload.getMultipleFileUploadJobs = fileUpload.getFileUploadJob = void 0;
const fs_1 = require$$0$3;
const stream_1$6 = require$$0$5;
...
fileUpload.getFileUploadJob = getFileUploadJob;
/**
...
fileUpload.getMultipleFileUploadJobs = getMultipleFileUploadJobs;
...
fileUpload.getFileData = getFileData;
...
fileUpload.getFileDataLength = getFileDataLength;
...
fileUpload.getFileDataAsStream = getFileDataAsStream;
/**
...
fileUpload.getAllFileUploadsToComplete = getAllFileUploadsToComplete;
// Validation
/**
...
fileUpload.warnIfNotUsingFilesUploadV2 = warnIfNotUsingFilesUploadV2;
/**
...
fileUpload.warnIfChannels = warnIfChannels;
/**
...
fileUpload.errorIfChannelsCsv = errorIfChannelsCsv;
/**
...
function errorIfInvalidOrMissingFileData(options) {
    ...
    /* eslint-disable @typescript-eslint/no-explicit-any */
    if (file && !(typeof file === 'string' || Buffer.isBuffer(file) || file instanceof stream_1$6.Readable)) {
...
fileUpload.errorIfInvalidOrMissingFileData = errorIfInvalidOrMissingFileData;
/**
 * @param options
 * @param logger
...
fileUpload.warnIfMissingOrInvalidFileNameAndDefault = warnIfMissingOrInvalidFileNameAndDefault;
/**
...
fileUpload.buildLegacyFileTypeWarning = buildLegacyFileTypeWarning;
...
fileUpload.buildMissingFileNameWarning = buildMissingFileNameWarning;
...
fileUpload.buildMissingExtensionWarning = buildMissingExtensionWarning;
function buildLegacyMethodWarning(method) {
    return `${method} may cause some issues like timeouts for relatively large files.`;
}
fileUpload.buildLegacyMethodWarning = buildLegacyMethodWarning;
...
fileUpload.buildGeneralFilesUploadWarning = buildGeneralFilesUploadWarning;
function buildFilesUploadMissingMessage() {
    return 'Something went wrong with processing file_uploads';
}
fileUpload.buildFilesUploadMissingMessage = buildFilesUploadMissingMessage;
...
fileUpload.buildChannelsWarning = buildChannelsWarning;
...
fileUpload.buildMultipleChannelsErrorMsg = buildMultipleChannelsErrorMsg;
...
fileUpload.buildInvalidFilesUploadParamError = buildInvalidFilesUploadParamError;

var hasRequiredWebClient;

function requireWebClient () {
	if (hasRequiredWebClient) return WebClient;
	hasRequiredWebClient = 1;
	(function (exports) {
		var __createBinding = (commonjsGlobal && commonjsGlobal.__createBinding) || (Object.create ? (function(o, m, k, k2) {
		    if (k2 === undefined) k2 = k;
		    var desc = Object.getOwnPropertyDescriptor(m, k);
		...
		const querystring_1 = require$$0$6;
		const path_1 = require$$0$4;
		const zlib_1 = __importDefault(require$$8);
		const util_1 = require$$1;
		const is_stream_1 = __importDefault(isStream$2.exports);
		const p_queue_1 = __importDefault(dist$3);
		const p_retry_1 = __importStar(pRetry$1.exports);
		...
		const retry_policies_1 = retryPolicies;
		const helpers_1 = __importDefault(helpers$1);
		const file_upload_1 = fileUpload;
		/*
		 * Helpers
		 */
...
(function (exports) {
	...
	var WebClient_1 = requireWebClient();
...
const constants_1$G = constants$1;
const utils$8 = utils$9;
const common$5 = common$6;
function read$1(directory, settings, callback) {
    if (!settings.stats && constants_1$G.IS_SUPPORT_READDIR_WITH_FILE_TYPES) {
...
const constants_1$F = constants$1;
const utils$7 = utils$9;
const common$4 = common$6;
function read(directory, settings) {
    if (!settings.stats && constants_1$F.IS_SUPPORT_READDIR_WITH_FILE_TYPES) {
...
const isIgnoredByIgnoreFiles = async (patterns, options) => {
	...
	const files = await Promise.all(
		paths.map(async filePath => ({
			filePath,
			content: await fs$b.promises.readFile(filePath, 'utf8'),
		})),
	);
...
const checkCwdOption = options => {
	...
	let stat;
	try {
		stat = fs$b.statSync(options.cwd);
	} catch {
		return;
	}
...
const getFilter$1 = async options => {
...
const base_1$G = base;
class DispatchActionsConfigurationObject extends base_1$G.CompositionObjectBase {
    constructor(params) {
        super();
...
const base_1$F = base;
class FilterObject extends base_1$F.CompositionObjectBase {
    constructor(params) {
        super();
...
class MarkdownObject extends base_1$E.CompositionObjectBase {
    constructor(text) {
        super();
        this.type = constants_1$E.ObjectType.Markdown;
        this.text = text;
    }
}
...
class PlainTextObject extends base_1$D.CompositionObjectBase {
    constructor(text) {
        super();
        this.type = constants_1$D.ObjectType.Text;
        this.text = text;
    }
}
...
buildHelpers.getDispatchActionsConfigurationObject = buildHelpers.getFilter = buildHelpers.getDateTimeIntegerFromDate = buildHelpers.getFormattedDate = buildHelpers.getFields = buildHelpers.getElementsForContext = buildHelpers.getMarkdownObject = buildHelpers.getStringFromNumber = buildHelpers.getPlainTextObject = buildHelpers.getBuilderResults = buildHelpers.getBuilderResult = void 0;
...
buildHelpers.getStringFromNumber = getStringFromNumber;
...
buildHelpers.getDateTimeIntegerFromDate = getDateTimeIntegerFromDate;
...
class Attachments extends lib_1$6.Builder {
    /**
    ...
    attachments(...attachments) {
        return this.append(attachments.flat(), constants_1$C.Prop.Attachments);
    }
}
appendMethods.Attachments = Attachments;
class Blocks extends lib_1$6.Builder {
    /**
     ...
     */
    blocks(...blocks) {
        return this.append(blocks.flat(), constants_1$C.Prop.Blocks);
    }
}
appendMethods.Blocks = Blocks;
class Elements extends lib_1$6.Builder {
    /**
     ...
     */
    elements(...elements) {
        return this.append(elements.flat(), constants_1$C.Prop.Elements);
    }
}
appendMethods.Elements = Elements;
class Fields extends lib_1$6.Builder {
    /**
     ...
     */
    fields(...fields) {
        return this.append(fields.flat(), constants_1$C.Prop.Fields);
    }
}
appendMethods.Fields = Fields;
class Filter extends lib_1$6.Builder {
    /**
     ...
     */
    filter(...filters) {
        return this.append(filters.flat(), constants_1$C.Prop.Filter);
    }
}
appendMethods.Filter = Filter;
class InitialChannels extends lib_1$6.Builder {
    /**
    ...
    initialChannels(...channelIds) {
        return this.append(channelIds.flat(), constants_1$C.Prop.InitialChannels);
    }
}
appendMethods.InitialChannels = InitialChannels;
class InitialConversations extends lib_1$6.Builder {
    /**
    ...
    initialConversations(...conversationIds) {
        return this.append(conversationIds.flat(), constants_1$C.Prop.InitialConversations);
    }
}
appendMethods.InitialConversations = InitialConversations;
class InitialOptions extends lib_1$6.Builder {
    /**
    ...
    initialOptions(...options) {
        return this.append(options.flat(), constants_1$C.Prop.InitialOptions);
    }
}
appendMethods.InitialOptions = InitialOptions;
class InitialUsers extends lib_1$6.Builder {
    /**
    ...
    initialUsers(...userIds) {
        return this.append(userIds.flat(), constants_1$C.Prop.InitialUsers);
    }
}
appendMethods.InitialUsers = InitialUsers;
class OptionGroups extends lib_1$6.Builder {
    /**
    ...
    optionGroups(...optionGroups) {
        return this.append(optionGroups.flat(), constants_1$C.Prop.OptionGroups);
    }
}
appendMethods.OptionGroups = OptionGroups;
class Options extends lib_1$6.Builder {
    /**
     ...
     */
    options(...options) {
        return this.append(options.flat(), constants_1$C.Prop.Options);
    }
}
...
class AsUser extends lib_1$5.Builder {
    /**
     ...
     */
    asUser(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.AsUser);
    }
}
configurationMethods.AsUser = AsUser;
class ClearOnClose extends lib_1$5.Builder {
    /**
    ...
    clearOnClose(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.ClearOnClose);
    }
}
configurationMethods.ClearOnClose = ClearOnClose;
class Danger extends lib_1$5.Builder {
    /**
     ...
     */
    danger(boolean = true) {
        return boolean ? this.set(constants_1$B.ButtonStyle.Danger, constants_1$B.Prop.Style) : this;
    }
}
configurationMethods.Danger = Danger;
class DefaultToCurrentConversation extends lib_1$5.Builder {
    /**
    ...
    defaultToCurrentConversation(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.DefaultToCurrentConversation);
    }
}
configurationMethods.DefaultToCurrentConversation = DefaultToCurrentConversation;
class DeleteOriginal extends lib_1$5.Builder {
    /**
    ...
    deleteOriginal(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.DeleteOriginal);
    }
}
configurationMethods.DeleteOriginal = DeleteOriginal;
class DispatchAction extends lib_1$5.Builder {
    /**
    ...
    dispatchAction(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.DispatchAction);
    }
}
configurationMethods.DispatchAction = DispatchAction;
class DispatchActionOnCharacterEntered extends lib_1$5.Builder {
    /**
    ...
    dispatchActionOnCharacterEntered(boolean = true) {
        return boolean ? this.set(constants_1$B.DispatchOnType.OnCharacterEntered, constants_1$B.Prop.OnCharacterEntered) : this;
    }
}
configurationMethods.DispatchActionOnCharacterEntered = DispatchActionOnCharacterEntered;
class DispatchActionOnEnterPressed extends lib_1$5.Builder {
    /**
    ...
    dispatchActionOnEnterPressed(boolean = true) {
        return boolean ? this.set(constants_1$B.DispatchOnType.OnEnterPressed, constants_1$B.Prop.OnEnterPressed) : this;
    }
}
configurationMethods.DispatchActionOnEnterPressed = DispatchActionOnEnterPressed;
class Ephemeral extends lib_1$5.Builder {
    /**
    ...
    ephemeral(boolean = true) {
        return boolean ? this.set(constants_1$B.ResponseType.Ephemeral, constants_1$B.Prop.ResponseType) : this;
    }
}
configurationMethods.Ephemeral = Ephemeral;
class ExcludeExternalSharedChannels extends lib_1$5.Builder {
    /**
    ...
    excludeExternalSharedChannels(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.ExcludeExternalSharedChannels);
    }
}
configurationMethods.ExcludeExternalSharedChannels = ExcludeExternalSharedChannels;
class ExcludeBotUsers extends lib_1$5.Builder {
    /**
    ...
    excludeBotUsers(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.ExcludeBotUsers);
    }
}
configurationMethods.ExcludeBotUsers = ExcludeBotUsers;
class FocusOnLoad extends lib_1$5.Builder {
    /**
    ...
    focusOnLoad(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.FocusOnLoad);
    }
}
configurationMethods.FocusOnLoad = FocusOnLoad;
class IgnoreMarkdown extends lib_1$5.Builder {
    /**
    ...
    ignoreMarkdown(boolean = false) {
        return this.set(boolean, constants_1$B.Prop.Mrkdwn);
    }
}
configurationMethods.IgnoreMarkdown = IgnoreMarkdown;
class InChannel extends lib_1$5.Builder {
    /**
    ...
    inChannel(boolean = true) {
        return boolean ? this.set(constants_1$B.ResponseType.InChannel, constants_1$B.Prop.ResponseType) : this;
    }
}
configurationMethods.InChannel = InChannel;
class Multiline extends lib_1$5.Builder {
    /**
    ...
    multiline(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.Multiline);
    }
}
configurationMethods.Multiline = Multiline;
class NotifyOnClose extends lib_1$5.Builder {
    /**
    ...
    notifyOnClose(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.NotifyOnClose);
    }
}
configurationMethods.NotifyOnClose = NotifyOnClose;
class Optional extends lib_1$5.Builder {
    /**
    ...
    optional(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.Optional);
    }
}
configurationMethods.Optional = Optional;
class Primary extends lib_1$5.Builder {
    /**
    ...
    primary(boolean = true) {
        return boolean ? this.set(constants_1$B.ButtonStyle.Primary, constants_1$B.Prop.Style) : this;
    }
}
configurationMethods.Primary = Primary;
class ReplaceOriginal extends lib_1$5.Builder {
    /**
    ...
    replaceOriginal(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.ReplaceOriginal);
    }
}
configurationMethods.ReplaceOriginal = ReplaceOriginal;
class ResponseUrlEnabled extends lib_1$5.Builder {
    /**
    ...
    responseUrlEnabled(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.ResponseUrlEnabled);
    }
}
configurationMethods.ResponseUrlEnabled = ResponseUrlEnabled;
class SubmitDisabled extends lib_1$5.Builder {
    /**
    ...
    submitDisabled(boolean = true) {
        return this.set(boolean, constants_1$B.Prop.SubmitDisabled);
    }
}
...
const lib_1$4 = lib;
class BuildToJSON extends lib_1$4.Builder {
    /**
...
class BuildToObject extends lib_1$4.Builder {
    /**
...
class End extends lib_1$4.Builder {
    /**
...
class GetAttachments extends lib_1$4.Builder {
    /**
...
class GetBlocks extends lib_1$4.Builder {
    /**
...
class GetPreviewUrl extends lib_1$4.Builder {
    /**
...
setMethods.VideoUrl = setMethods.Value = setMethods.Url = setMethods.Ts = setMethods.TitleUrl = setMethods.Title = setMethods.ThumbnailUrl = setMethods.ThreadTs = setMethods.Text = setMethods.Submit = setMethods.ProviderName = setMethods.ProviderIconUrl = setMethods.PrivateMetaData = setMethods.PostAt = setMethods.Placeholder = setMethods.MinValue = setMethods.MinLength = setMethods.MinQueryLength = setMethods.MaxValue = setMethods.MaxSelectedItems = setMethods.MaxLength = setMethods.Label = setMethods.IsDecimalAllowed = setMethods.InitialValue = setMethods.InitialUser = setMethods.InitialTime = setMethods.InitialOption = setMethods.InitialDateTime = setMethods.InitialDate = setMethods.InitialConversation = setMethods.InitialChannel = setMethods.ImageUrl = setMethods.Hint = setMethods.Fallback = setMethods.ExternalId = setMethods.Element = setMethods.Description = setMethods.Deny = setMethods.Confirm = setMethods.Color = setMethods.Close = setMethods.Channel = setMethods.CallbackId = setMethods.BlockId = setMethods.AuthorName = setMethods.AltText = setMethods.ActionId = setMethods.Accessory = setMethods.AccessibilityLabel = void 0;
const lib_1$3 = lib;
const constants_1$A = constants;
class AccessibilityLabel extends lib_1$3.Builder {
    /**
    ...
    accessibilityLabel(label) {
        return this.set(label, constants_1$A.Prop.AccessibilityLabel);
    }
}
setMethods.AccessibilityLabel = AccessibilityLabel;
class Accessory extends lib_1$3.Builder {
    /**
     ...
     */
    accessory(element) {
        return this.set(element, constants_1$A.Prop.Accessory);
    }
}
setMethods.Accessory = Accessory;
class ActionId extends lib_1$3.Builder {
    /**
     ...
     */
    actionId(actionId) {
        return this.set(actionId, constants_1$A.Prop.ActionId);
    }
}
setMethods.ActionId = ActionId;
class AltText extends lib_1$3.Builder {
    /**
     ...
     */
    altText(altText) {
        return this.set(altText, constants_1$A.Prop.AltText);
    }
}
setMethods.AltText = AltText;
class AuthorName extends lib_1$3.Builder {
    /**
    ...
    authorName(authorName) {
        return this.set(authorName, constants_1$A.Prop.AuthorName);
    }
}
setMethods.AuthorName = AuthorName;
class BlockId extends lib_1$3.Builder {
    /**
     ...
     */
    blockId(blockId) {
        return this.set(blockId, constants_1$A.Prop.BlockId);
    }
}
setMethods.BlockId = BlockId;
class CallbackId extends lib_1$3.Builder {
    /**
    ...
    callbackId(callbackId) {
        return this.set(callbackId, constants_1$A.Prop.CallbackId);
    }
}
setMethods.CallbackId = CallbackId;
class Channel extends lib_1$3.Builder {
    /**
     ...
     */
    channel(channel) {
        return this.set(channel, constants_1$A.Prop.Channel);
    }
}
setMethods.Channel = Channel;
class Close extends lib_1$3.Builder {
    /**
    ...
    close(closeButtonText) {
        return this.set(closeButtonText, constants_1$A.Prop.Close);
    }
}
setMethods.Close = Close;
class Color extends lib_1$3.Builder {
    /**
     ...
     */
    color(color) {
        return this.set(color, constants_1$A.Prop.Color);
    }
}
setMethods.Color = Color;
class Confirm extends lib_1$3.Builder {
    /**
     ...
     */
    confirm(value) {
        return this.set(value, constants_1$A.Prop.Confirm);
    }
}
setMethods.Confirm = Confirm;
class Deny extends lib_1$3.Builder {
    /**
     ...
     */
    deny(text) {
        return this.set(text, constants_1$A.Prop.Deny);
    }
}
setMethods.Deny = Deny;
class Description extends lib_1$3.Builder {
    /**
     ...
     */
    description(description) {
        return this.set(description, constants_1$A.Prop.Description);
    }
}
setMethods.Description = Description;
class Element extends lib_1$3.Builder {
    /**
     ...
     */
    element(element) {
        return this.set(element, constants_1$A.Prop.Element);
    }
}
setMethods.Element = Element;
class ExternalId extends lib_1$3.Builder {
    /**
    ...
    externalId(externalId) {
        return this.set(externalId, constants_1$A.Prop.ExternalId);
    }
}
setMethods.ExternalId = ExternalId;
class Fallback extends lib_1$3.Builder {
    /**
     ...
     */
    fallback(text) {
        return this.set(text, constants_1$A.Prop.Fallback);
    }
}
setMethods.Fallback = Fallback;
class Hint extends lib_1$3.Builder {
    /**
     ...
     */
    hint(hint) {
        return this.set(hint, constants_1$A.Prop.Hint);
    }
}
setMethods.Hint = Hint;
class ImageUrl extends lib_1$3.Builder {
    /**
     ...
     */
    imageUrl(url) {
        return this.set(url, constants_1$A.Prop.ImageUrl);
    }
}
setMethods.ImageUrl = ImageUrl;
class InitialChannel extends lib_1$3.Builder {
    /**
    ...
    initialChannel(channelId) {
        return this.set(channelId, constants_1$A.Prop.InitialChannel);
    }
}
setMethods.InitialChannel = InitialChannel;
class InitialConversation extends lib_1$3.Builder {
    /**
    ...
    initialConversation(conversationId) {
        return this.set(conversationId, constants_1$A.Prop.InitialConversation);
    }
}
setMethods.InitialConversation = InitialConversation;
class InitialDate extends lib_1$3.Builder {
    /**
     ...
     */
    initialDate(date) {
        return this.set(date, constants_1$A.Prop.InitialDate);
    }
}
setMethods.InitialDate = InitialDate;
class InitialDateTime extends lib_1$3.Builder {
    /**
    ...
    initialDateTime(date) {
        return this.set(date, constants_1$A.Prop.InitialDateTime);
    }
}
setMethods.InitialDateTime = InitialDateTime;
class InitialOption extends lib_1$3.Builder {
    /**
    ...
    initialOption(option) {
        return this.set(option, constants_1$A.Prop.InitialOption);
    }
}
setMethods.InitialOption = InitialOption;
class InitialTime extends lib_1$3.Builder {
    /**
     ...
     */
    initialTime(time) {
        return this.set(time, constants_1$A.Prop.InitialTime);
    }
}
setMethods.InitialTime = InitialTime;
class InitialUser extends lib_1$3.Builder {
    /**
     ...
     */
    initialUser(userId) {
        return this.set(userId, constants_1$A.Prop.InitialUser);
    }
}
setMethods.InitialUser = InitialUser;
class InitialValue extends lib_1$3.Builder {
    /**
     ...
     */
    initialValue(value) {
        return this.set(value, constants_1$A.Prop.InitialValue);
    }
}
setMethods.InitialValue = InitialValue;
class IsDecimalAllowed extends lib_1$3.Builder {
    /**
    ...
    isDecimalAllowed(value) {
        return this.set(value, constants_1$A.Prop.IsDecimalAllowed);
    }
}
setMethods.IsDecimalAllowed = IsDecimalAllowed;
class Label extends lib_1$3.Builder {
    /**
     ...
     */
    label(label) {
        return this.set(label, constants_1$A.Prop.Label);
    }
}
setMethods.Label = Label;
class MaxLength extends lib_1$3.Builder {
    /**
     ...
     */
    maxLength(length) {
        return this.set(length, constants_1$A.Prop.MaxLength);
    }
}
setMethods.MaxLength = MaxLength;
class MaxSelectedItems extends lib_1$3.Builder {
    /**
    ...
    maxSelectedItems(quantity) {
        return this.set(quantity, constants_1$A.Prop.MaxSelectedItems);
    }
}
setMethods.MaxSelectedItems = MaxSelectedItems;
class MaxValue extends lib_1$3.Builder {
    /**
     ...
     */
    maxValue(value) {
        return this.set(value, constants_1$A.Prop.MaxValue);
    }
}
setMethods.MaxValue = MaxValue;
class MinQueryLength extends lib_1$3.Builder {
    /**
    ...
    minQueryLength(length) {
        return this.set(length, constants_1$A.Prop.MinQueryLength);
    }
}
setMethods.MinQueryLength = MinQueryLength;
class MinLength extends lib_1$3.Builder {
    /**
     ...
     */
    minLength(length) {
        return this.set(length, constants_1$A.Prop.MinLength);
    }
}
setMethods.MinLength = MinLength;
class MinValue extends lib_1$3.Builder {
    /**
     ...
     */
    minValue(value) {
        return this.set(value, constants_1$A.Prop.MinValue);
    }
}
setMethods.MinValue = MinValue;
class Placeholder extends lib_1$3.Builder {
    /**
    ...
    placeholder(placeholder) {
        return this.set(placeholder, constants_1$A.Prop.Placeholder);
    }
}
setMethods.Placeholder = Placeholder;
class PostAt extends lib_1$3.Builder {
    /**
     ...
     */
    postAt(timestamp) {
        return this.set(timestamp, constants_1$A.Prop.PostAt);
    }
}
setMethods.PostAt = PostAt;
class PrivateMetaData extends lib_1$3.Builder {
    /**
    ...
    privateMetaData(privateMetaData) {
        return this.set(privateMetaData, constants_1$A.Prop.PrivateMetaData);
    }
}
setMethods.PrivateMetaData = PrivateMetaData;
class ProviderIconUrl extends lib_1$3.Builder {
    /**
    ...
    providerIconUrl(providerIconUrl) {
        return this.set(providerIconUrl, constants_1$A.Prop.ProviderIconUrl);
    }
}
setMethods.ProviderIconUrl = ProviderIconUrl;
class ProviderName extends lib_1$3.Builder {
    /**
    ...
    providerName(providerName) {
        return this.set(providerName, constants_1$A.Prop.ProviderName);
    }
}
setMethods.ProviderName = ProviderName;
class Submit extends lib_1$3.Builder {
    /**
     ...
     */
    submit(text) {
        return this.set(text, constants_1$A.Prop.Submit);
    }
}
setMethods.Submit = Submit;
class Text extends lib_1$3.Builder {
    /**
     ...
     */
    text(text) {
        return this.set(text, constants_1$A.Prop.Text);
    }
}
setMethods.Text = Text;
class ThreadTs extends lib_1$3.Builder {
    /**
     ...
     */
    threadTs(threadTs) {
        return this.set(threadTs, constants_1$A.Prop.ThreadTs);
    }
}
setMethods.ThreadTs = ThreadTs;
class ThumbnailUrl extends lib_1$3.Builder {
    /**
    ...
    thumbnailUrl(thumbnailUrl) {
        return this.set(thumbnailUrl, constants_1$A.Prop.ThumbnailUrl);
    }
}
setMethods.ThumbnailUrl = ThumbnailUrl;
class Title extends lib_1$3.Builder {
    /**
     ...
     */
    title(title) {
        return this.set(title, constants_1$A.Prop.Title);
    }
}
setMethods.Title = Title;
class TitleUrl extends lib_1$3.Builder {
    /**
     ...
     */
    titleUrl(titleUrl) {
        return this.set(titleUrl, constants_1$A.Prop.TitleUrl);
    }
}
setMethods.TitleUrl = TitleUrl;
class Ts extends lib_1$3.Builder {
    /**
     ...
     */
    ts(ts) {
        return this.set(ts, constants_1$A.Prop.Ts);
    }
}
setMethods.Ts = Ts;
class Url extends lib_1$3.Builder {
    /**
     ...
     */
    url(url) {
        return this.set(url, constants_1$A.Prop.Url);
    }
}
setMethods.Url = Url;
class Value extends lib_1$3.Builder {
    /**
     ...
     */
    value(value) {
        return this.set(value, constants_1$A.Prop.Value);
    }
}
setMethods.Value = Value;
class VideoUrl extends lib_1$3.Builder {
    /**
     ...
     */
    videoUrl(videoUrl) {
        return this.set(videoUrl, constants_1$A.Prop.VideoUrl);
    }
}
setMethods.VideoUrl = VideoUrl;
 ...
 */
class AttachmentBuilder extends base_1$C.BitBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$C.SlackDto, {
            blocks: helpers_1$C.getBuilderResults(this.props.blocks),
        });
    }
}
attachment.AttachmentBuilder = AttachmentBuilder;
helpers_1$C.applyMixins(AttachmentBuilder, [
    methods_1$C.Blocks,
    methods_1$C.Color,
    methods_1$C.End,
    methods_1$C.Fallback,
]);
 ...
 */
class ConfirmationDialogBuilder extends base_1$B.BitBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$B.SlackDto, {
            text: helpers_1$B.getMarkdownObject(this.props.text),
            title: helpers_1$B.getPlainTextObject(this.props.title),
            confirm: helpers_1$B.getPlainTextObject(this.props.confirm),
            deny: helpers_1$B.getPlainTextObject(this.props.deny),
        });
    }
}
confirmationDialog.ConfirmationDialogBuilder = ConfirmationDialogBuilder;
helpers_1$B.applyMixins(ConfirmationDialogBuilder, [
    methods_1$B.Confirm,
    methods_1$B.Danger,
    methods_1$B.Deny,
    methods_1$B.End,
 ...
 * @@displayName Option
 */
class OptionBuilder extends base_1$A.BitBuilderBase {
    /** @internal */
    build({ isMarkdown } = { isMarkdown: false }) {
        return this.getResult(dto_1$A.SlackDto, {
            text: isMarkdown
                ? helpers_1$A.getMarkdownObject(this.props.text)
                : helpers_1$A.getPlainTextObject(this.props.text),
            description: isMarkdown
                ? helpers_1$A.getMarkdownObject(this.props.description)
                : helpers_1$A.getPlainTextObject(this.props.description),
        });
    }
}
option.OptionBuilder = OptionBuilder;
helpers_1$A.applyMixins(OptionBuilder, [
    methods_1$A.Description,
    methods_1$A.End,
    methods_1$A.Text,
    methods_1$A.Url,
    methods_1$A.Value,
...
const base_1$z = base;
const dto_1$z = dto;
const helpers_1$z = helpers;
const methods_1$z = methods;
/**
 * @@link https://api.slack.com/reference/block-kit/composition-objects#option_group
 * @@displayName Option Group
 */
class OptionGroupBuilder extends base_1$z.BitBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$z.SlackDto, {
            label: helpers_1$z.getPlainTextObject(this.props.label),
            options: helpers_1$z.getBuilderResults(this.props.options),
        });
    }
}
optionGroup.OptionGroupBuilder = OptionGroupBuilder;
helpers_1$z.applyMixins(OptionGroupBuilder, [
    methods_1$z.End,
    methods_1$z.Label,
    methods_1$z.Options,
]);
...
const dto_1$y = dto;
const helpers_1$y = helpers;
const methods_1$y = methods;
/**
 * @@link https://api.slack.com/reference/block-kit/blocks#actions
 * @@displayName Actions
 */
class ActionsBuilder extends base_1$y.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$y.SlackBlockDto, {
            type: constants_1$z.BlockType.Actions,
            elements: helpers_1$y.getBuilderResults(this.props.elements),
        });
    }
}
actions.ActionsBuilder = ActionsBuilder;
helpers_1$y.applyMixins(ActionsBuilder, [
    methods_1$y.BlockId,
    methods_1$y.End,
    methods_1$y.Elements,
]);
 ...
 * @@displayName Context
 */
class ContextBuilder extends base_1$x.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$x.SlackBlockDto, {
            type: constants_1$y.BlockType.Context,
            elements: helpers_1$x.getElementsForContext(this.props.elements),
        });
    }
}
context.ContextBuilder = ContextBuilder;
helpers_1$x.applyMixins(ContextBuilder, [
    methods_1$x.BlockId,
    methods_1$x.Elements,
    methods_1$x.End,
]);
 ...
 * @@displayName Divider
 */
class DividerBuilder extends base_1$w.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$w.SlackBlockDto, {
            type: constants_1$x.BlockType.Divider,
        });
    }
}
divider.DividerBuilder = DividerBuilder;
helpers_1$w.applyMixins(DividerBuilder, [
    methods_1$w.BlockId,
    methods_1$w.End,
]);
 ...
 * @@displayName File
 */
class FileBuilder extends base_1$v.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$v.SlackBlockDto, {
            type: constants_1$w.BlockType.File,
            source: constants_1$w.FileType.Remote,
        });
    }
}
file.FileBuilder = FileBuilder;
helpers_1$v.applyMixins(FileBuilder, [
    methods_1$v.BlockId,
    methods_1$v.End,
    methods_1$v.ExternalId,
]);
 ...
 * @@displayName Header
 */
class HeaderBuilder extends base_1$u.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$u.SlackBlockDto, {
            type: constants_1$v.BlockType.Header,
            text: helpers_1$u.getPlainTextObject(this.props.text),
        });
    }
}
header.HeaderBuilder = HeaderBuilder;
helpers_1$u.applyMixins(HeaderBuilder, [
    methods_1$u.BlockId,
    methods_1$u.End,
    methods_1$u.Text,
]);
 ...
 * @@displayName Image
 */
class ImageBuilder extends base_1$t.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$t.SlackBlockDto, {
            type: constants_1$u.BlockType.Image,
            title: helpers_1$t.getPlainTextObject(this.props.title),
        });
    }
}
image.ImageBuilder = ImageBuilder;
helpers_1$t.applyMixins(ImageBuilder, [
    methods_1$t.AltText,
    methods_1$t.BlockId,
    methods_1$t.End,
    methods_1$t.ImageUrl,
    methods_1$t.Title,
]);
 ...
 * @@displayName Input
 */
class InputBuilder extends base_1$s.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$s.SlackBlockDto, {
            type: constants_1$t.BlockType.Input,
            label: helpers_1$s.getPlainTextObject(this.props.label),
            hint: helpers_1$s.getPlainTextObject(this.props.hint),
            element: helpers_1$s.getBuilderResult(this.props.element),
        });
    }
}
input.InputBuilder = InputBuilder;
helpers_1$s.applyMixins(InputBuilder, [
    methods_1$s.BlockId,
    methods_1$s.DispatchAction,
    methods_1$s.Element,
    methods_1$s.End,
    methods_1$s.Hint,
    methods_1$s.Label,
    methods_1$s.Optional,
]);
 ...
 * @@displayName Section
 */
class SectionBuilder extends base_1$r.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$r.SlackBlockDto, {
            type: constants_1$s.BlockType.Section,
            text: helpers_1$r.getMarkdownObject(this.props.text),
            fields: helpers_1$r.getFields(this.props.fields),
            accessory: helpers_1$r.getBuilderResult(this.props.accessory),
        });
    }
}
section.SectionBuilder = SectionBuilder;
helpers_1$r.applyMixins(SectionBuilder, [
    methods_1$r.Accessory,
    methods_1$r.BlockId,
    methods_1$r.End,
    methods_1$r.Fields,
    methods_1$r.Text,
 ...
 * @@displayName Video
 */
class VideoBuilder extends base_1$q.BlockBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$q.SlackBlockDto, {
            type: constants_1$r.BlockType.Video,
            description: helpers_1$q.getPlainTextObject(this.props.description),
            title: helpers_1$q.getPlainTextObject(this.props.title),
        });
    }
}
video.VideoBuilder = VideoBuilder;
helpers_1$q.applyMixins(VideoBuilder, [
    methods_1$q.AltText,
    methods_1$q.AuthorName,
    methods_1$q.BlockId,
    methods_1$q.Description,
    methods_1$q.End,
    methods_1$q.ProviderIconUrl,
    methods_1$q.ProviderName,
    methods_1$q.ThumbnailUrl,
    methods_1$q.Title,
    methods_1$q.TitleUrl,
    methods_1$q.VideoUrl,
]);

/* eslint-disable max-len */
Object.defineProperty(blocks$1, "__esModule", { value: true });
blocks$1.Blocks = blocks$1.Video = blocks$1.Section = blocks$1.Input = blocks$1.Image = blocks$1.Header = blocks$1.File = blocks$1.Divider = blocks$1.Context = blocks$1.Actions = void 0;
 ...
 * @@displayName Button
 */
class ButtonBuilder extends base_1$p.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$p.SlackElementDto, {
            type: constants_1$q.ElementType.Button,
            confirm: helpers_1$p.getBuilderResult(this.props.confirm),
            text: helpers_1$p.getPlainTextObject(this.props.text),
        });
    }
}
button.ButtonBuilder = ButtonBuilder;
helpers_1$p.applyMixins(ButtonBuilder, [
    methods_1$p.AccessibilityLabel,
    methods_1$p.ActionId,
    methods_1$p.Confirm,
    methods_1$p.Danger,
    methods_1$p.End,
    methods_1$p.Primary,
 ...
 */
class ChannelMultiSelectBuilder extends base_1$o.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$o.SlackElementDto, {
            type: constants_1$p.ElementType.ChannelsMultiSelect,
            placeholder: helpers_1$o.getPlainTextObject(this.props.placeholder),
            confirm: helpers_1$o.getBuilderResult(this.props.confirm),
        });
    }
}
channelMultiSelect.ChannelMultiSelectBuilder = ChannelMultiSelectBuilder;
helpers_1$o.applyMixins(ChannelMultiSelectBuilder, [
    methods_1$o.ActionId,
    methods_1$o.Confirm,
    methods_1$o.End,
    methods_1$o.FocusOnLoad,
    methods_1$o.InitialChannels,
    methods_1$o.MaxSelectedItems,
    methods_1$o.Placeholder,
]);
 ...
 */
class ChannelSelectBuilder extends base_1$n.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$n.SlackElementDto, {
            type: constants_1$o.ElementType.ChannelSelect,
            placeholder: helpers_1$n.getPlainTextObject(this.props.placeholder),
            confirm: helpers_1$n.getBuilderResult(this.props.confirm),
        });
    }
}
channelSelect.ChannelSelectBuilder = ChannelSelectBuilder;
helpers_1$n.applyMixins(ChannelSelectBuilder, [
    methods_1$n.ActionId,
    methods_1$n.Confirm,
    methods_1$n.End,
    methods_1$n.FocusOnLoad,
    methods_1$n.InitialChannel,
    methods_1$n.Placeholder,
    methods_1$n.ResponseUrlEnabled,
]);
 ...
 */
class CheckboxesBuilder extends base_1$m.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$m.SlackElementDto, {
            type: constants_1$n.ElementType.Checkboxes,
            options: helpers_1$m.getBuilderResults(this.props.options, { isMarkdown: true }),
            initialOptions: helpers_1$m.getBuilderResults(this.props.initialOptions, { isMarkdown: true }),
            confirm: helpers_1$m.getBuilderResult(this.props.confirm),
        });
    }
}
checkboxes.CheckboxesBuilder = CheckboxesBuilder;
helpers_1$m.applyMixins(CheckboxesBuilder, [
    methods_1$m.ActionId,
    methods_1$m.Confirm,
    methods_1$m.End,
    methods_1$m.FocusOnLoad,
    methods_1$m.InitialOptions,
    methods_1$m.Options,
]);
 ...
 */
class ConversationMultiSelectBuilder extends base_1$l.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$l.SlackElementDto, {
            type: constants_1$m.ElementType.ConversationsMultiSelect,
            placeholder: helpers_1$l.getPlainTextObject(this.props.placeholder),
            confirm: helpers_1$l.getBuilderResult(this.props.confirm),
            filter: helpers_1$l.getFilter(this.props),
        });
    }
}
conversationMultiSelect.ConversationMultiSelectBuilder = ConversationMultiSelectBuilder;
helpers_1$l.applyMixins(ConversationMultiSelectBuilder, [
    methods_1$l.ActionId,
    methods_1$l.Confirm,
    methods_1$l.DefaultToCurrentConversation,
    methods_1$l.End,
    methods_1$l.ExcludeBotUsers,
    methods_1$l.ExcludeExternalSharedChannels,
    methods_1$l.Filter,
    methods_1$l.FocusOnLoad,
    methods_1$l.InitialConversations,
    methods_1$l.MaxSelectedItems,
    methods_1$l.Placeholder,
]);
 ...
 */
class ConversationSelectBuilder extends base_1$k.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$k.SlackElementDto, {
            type: constants_1$l.ElementType.ConversationSelect,
            placeholder: helpers_1$k.getPlainTextObject(this.props.placeholder),
            confirm: helpers_1$k.getBuilderResult(this.props.confirm),
            filter: helpers_1$k.getFilter(this.props),
        });
    }
}
conversationSelect.ConversationSelectBuilder = ConversationSelectBuilder;
helpers_1$k.applyMixins(ConversationSelectBuilder, [
    methods_1$k.ActionId,
    methods_1$k.Confirm,
    methods_1$k.DefaultToCurrentConversation,
    methods_1$k.End,
    methods_1$k.ExcludeBotUsers,
    methods_1$k.ExcludeExternalSharedChannels,
    methods_1$k.Filter,
    methods_1$k.FocusOnLoad,
    methods_1$k.InitialConversation,
    methods_1$k.Placeholder,
    methods_1$k.ResponseUrlEnabled,
    methods_1$k.Placeholder,
]);
 ...
 */
class DatePickerBuilder extends base_1$j.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$j.SlackElementDto, {
            type: constants_1$k.ElementType.DatePicker,
            placeholder: helpers_1$j.getPlainTextObject(this.props.placeholder),
            initialDate: helpers_1$j.getFormattedDate(this.props.initialDate),
            confirm: helpers_1$j.getBuilderResult(this.props.confirm),
        });
    }
}
datePicker.DatePickerBuilder = DatePickerBuilder;
helpers_1$j.applyMixins(DatePickerBuilder, [
    methods_1$j.ActionId,
    methods_1$j.Confirm,
    methods_1$j.End,
    methods_1$j.FocusOnLoad,
    methods_1$j.InitialDate,
    methods_1$j.Placeholder,
]);

var dateTimePicker = {};

Object.defineProperty(dateTimePicker, "__esModule", { value: true });
dateTimePicker.DateTimePickerBuilder = void 0;
const base_1$i = base;
 ...
 * @@displayName Date Picker
 */
class DateTimePickerBuilder extends base_1$i.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$i.SlackElementDto, {
            type: constants_1$j.ElementType.DateTimePicker,
            initialDateTime: helpers_1$i.getDateTimeIntegerFromDate(this.props.initialDateTime),
            confirm: helpers_1$i.getBuilderResult(this.props.confirm),
        });
    }
}
dateTimePicker.DateTimePickerBuilder = DateTimePickerBuilder;
helpers_1$i.applyMixins(DateTimePickerBuilder, [
    methods_1$i.ActionId,
    methods_1$i.Confirm,
    methods_1$i.End,
    methods_1$i.FocusOnLoad,
    methods_1$i.InitialDateTime,
]);

var emailInput = {};

Object.defineProperty(emailInput, "__esModule", { value: true });
emailInput.EmailInputBuilder = void 0;
const base_1$h = base;
 ...
 */
class EmailInputBuilder extends base_1$h.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$h.SlackElementDto, {
            type: constants_1$i.ElementType.EmailInput,
            placeholder: helpers_1$h.getPlainTextObject(this.props.placeholder),
            dispatchActionConfig: helpers_1$h.getDispatchActionsConfigurationObject(this.props),
        });
    }
}
emailInput.EmailInputBuilder = EmailInputBuilder;
helpers_1$h.applyMixins(EmailInputBuilder, [
    methods_1$h.ActionId,
    methods_1$h.DispatchActionOnCharacterEntered,
    methods_1$h.DispatchActionOnEnterPressed,
    methods_1$h.End,
    methods_1$h.FocusOnLoad,
    methods_1$h.InitialValue,
 ...
 */
class ExternalMultiSelectBuilder extends base_1$g.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$g.SlackElementDto, {
            type: constants_1$h.ElementType.ExternalMultiSelect,
            placeholder: helpers_1$g.getPlainTextObject(this.props.placeholder),
            initialOptions: helpers_1$g.getBuilderResults(this.props.initialOptions),
...
helpers_1$g.applyMixins(ExternalMultiSelectBuilder, [
    methods_1$g.ActionId,
    methods_1$g.Confirm,
    methods_1$g.End,
    methods_1$g.FocusOnLoad,
    methods_1$g.InitialOptions,
    methods_1$g.MaxSelectedItems,
    methods_1$g.MinQueryLength,
 ...
 */
class ExternalSelectBuilder extends base_1$f.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$f.SlackElementDto, {
            type: constants_1$g.ElementType.ExternalSelect,
            placeholder: helpers_1$f.getPlainTextObject(this.props.placeholder),
            initialOption: helpers_1$f.getBuilderResult(this.props.initialOption),
            confirm: helpers_1$f.getBuilderResult(this.props.confirm),
        });
    }
}
externalSelect.ExternalSelectBuilder = ExternalSelectBuilder;
helpers_1$f.applyMixins(ExternalSelectBuilder, [
    methods_1$f.ActionId,
    methods_1$f.Confirm,
    methods_1$f.End,
    methods_1$f.FocusOnLoad,
    methods_1$f.InitialOption,
    methods_1$f.MinQueryLength,
 ...
 * @@displayName Image
 */
class ImgBuilder extends base_1$e.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$e.SlackElementDto, {
            type: constants_1$f.ElementType.Image,
        });
    }
}
img.ImgBuilder = ImgBuilder;
helpers_1$e.applyMixins(ImgBuilder, [
    methods_1$e.AltText,
    methods_1$e.ImageUrl,
    methods_1$e.End,
]);

var numberInput = {};

Object.defineProperty(numberInput, "__esModule", { value: true });
numberInput.NumberInputBuilder = void 0;
const base_1$d = base;
const constants_1$e = constants;
const dto_1$d = dto;
 ...
 * @@displayName Email Input
 */
class NumberInputBuilder extends base_1$d.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$d.SlackElementDto, {
            type: constants_1$e.ElementType.NumberInput,
            initialValue: helpers_1$d.getStringFromNumber(this.props.initialValue),
            maxValue: helpers_1$d.getStringFromNumber(this.props.maxValue),
            minValue: helpers_1$d.getStringFromNumber(this.props.minValue),
            placeholder: helpers_1$d.getPlainTextObject(this.props.placeholder),
            dispatchActionConfig: helpers_1$d.getDispatchActionsConfigurationObject(this.props),
        });
    }
}
numberInput.NumberInputBuilder = NumberInputBuilder;
helpers_1$d.applyMixins(NumberInputBuilder, [
    methods_1$d.ActionId,
    methods_1$d.DispatchActionOnCharacterEntered,
    methods_1$d.DispatchActionOnEnterPressed,
    methods_1$d.End,
    methods_1$d.FocusOnLoad,
    methods_1$d.InitialValue,
    methods_1$d.IsDecimalAllowed,
    methods_1$d.MaxValue,
    methods_1$d.MinValue,
 ...
 */
class OverflowMenuBuilder extends base_1$c.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$c.SlackElementDto, {
            type: constants_1$d.ElementType.Overflow,
            options: helpers_1$c.getBuilderResults(this.props.options),
            confirm: helpers_1$c.getBuilderResult(this.props.confirm),
        });
    }
}
overflowMenu.OverflowMenuBuilder = OverflowMenuBuilder;
helpers_1$c.applyMixins(OverflowMenuBuilder, [
    methods_1$c.ActionId,
    methods_1$c.Confirm,
    methods_1$c.End,
    methods_1$c.Options,
]);
 ...
 */
class RadioButtonsBuilder extends base_1$b.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$b.SlackElementDto, {
            type: constants_1$c.ElementType.RadioButtons,
            options: helpers_1$b.getBuilderResults(this.props.options, { isMarkdown: true }),
            initialOption: helpers_1$b.getBuilderResult(this.props.initialOption, { isMarkdown: true }),
            confirm: helpers_1$b.getBuilderResult(this.props.confirm),
        });
    }
}
radioButtons.RadioButtonsBuilder = RadioButtonsBuilder;
helpers_1$b.applyMixins(RadioButtonsBuilder, [
    methods_1$b.ActionId,
    methods_1$b.Confirm,
    methods_1$b.End,
    methods_1$b.FocusOnLoad,
    methods_1$b.InitialOption,
    methods_1$b.Options,
]);
...
const helpers_1$a = helpers;
const methods_1$a = methods;
class StaticMultiSelectBuilder extends base_1$a.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$a.SlackElementDto, {
            type: constants_1$b.ElementType.StaticMultiSelect,
            placeholder: helpers_1$a.getPlainTextObject(this.props.placeholder),
            options: helpers_1$a.getBuilderResults(this.props.options),
            initialOptions: helpers_1$a.getBuilderResults(this.props.initialOptions),
            optionGroups: helpers_1$a.getBuilderResults(this.props.optionGroups),
...
helpers_1$a.applyMixins(StaticMultiSelectBuilder, [
    methods_1$a.ActionId,
    methods_1$a.Confirm,
    methods_1$a.End,
    methods_1$a.FocusOnLoad,
    methods_1$a.InitialOptions,
    methods_1$a.MaxSelectedItems,
    methods_1$a.OptionGroups,
    methods_1$a.Options,
    methods_1$a.Placeholder,
]);
 ...
 */
class StaticSelectBuilder extends base_1$9.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$9.SlackElementDto, {
            type: constants_1$a.ElementType.StaticSelect,
 ...
 */
class TextInputBuilder extends base_1$8.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$8.SlackElementDto, {
            type: constants_1$9.ElementType.TextInput,
            placeholder: helpers_1$8.getPlainTextObject(this.props.placeholder),
            dispatchActionConfig: helpers_1$8.getDispatchActionsConfigurationObject(this.props),
        });
    }
}
textInput.TextInputBuilder = TextInputBuilder;
helpers_1$8.applyMixins(TextInputBuilder, [
    methods_1$8.ActionId,
    methods_1$8.DispatchActionOnCharacterEntered,
    methods_1$8.DispatchActionOnEnterPressed,
    methods_1$8.End,
    methods_1$8.FocusOnLoad,
    methods_1$8.InitialValue,
    methods_1$8.MaxLength,
    methods_1$8.MinLength,
    methods_1$8.Multiline,
 ...
 */
class TimePickerBuilder extends base_1$7.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$7.SlackElementDto, {
            type: constants_1$8.ElementType.TimePicker,
            placeholder: helpers_1$7.getPlainTextObject(this.props.placeholder),
            confirm: helpers_1$7.getBuilderResult(this.props.confirm),
        });
    }
}
timepicker.TimePickerBuilder = TimePickerBuilder;
helpers_1$7.applyMixins(TimePickerBuilder, [
    methods_1$7.ActionId,
    methods_1$7.Confirm,
    methods_1$7.End,
    methods_1$7.FocusOnLoad,
    methods_1$7.InitialTime,
    methods_1$7.Placeholder,
]);

var urlInput = {};

Object.defineProperty(urlInput, "__esModule", { value: true });
urlInput.URLInputBuilder = void 0;
const base_1$6 = base;
 ...
 */
class URLInputBuilder extends base_1$6.ElementBuilderBase {
    /** @internal */
    build() {
        return this.getResult(dto_1$6.SlackElementDto, {
            type: constants_1$7.ElementType.URLInput,
            placeholder: helpers_1$6.getPlainTextObject(this.props.placeholder),
            dispatchActionConfig: helpers_1$6.getDispatchActionsConfigurationObject(this.props),
        });
    }
}
urlInput.URLInputBuilder = URLInputBuilder;
helpers_1$6.applyMixins(URLInputBuilder, [
    methods_1$6.ActionId,
    methods_1$6.DispatchActionOnCharacterEntered,
    methods_1$6.DispatchActionOnEnterPressed,
    methods_1$6.End,
    methods_1$6.FocusOnLoad,
    methods_1$6.InitialValue,
...
elements$1.Elements = elements$1.UserSelect = elements$1.UserMultiSelect = elements$1.URLInput = elements$1.TimePicker = elements$1.TextInput = elements$1.StaticSelect = elements$1.StaticMultiSelect = elements$1.RadioButtons = elements$1.OverflowMenu = elements$1.NumberInput = elements$1.Img = elements$1.ExternalSelect = elements$1.ExternalMultiSelect = elements$1.EmailInput = elements$1.DateTimePicker = elements$1.DatePicker = elements$1.ConversationSelect = elements$1.ConversationMultiSelect = elements$1.Checkboxes = elements$1.ChannelSelect = elements$1.ChannelMultiSelect = elements$1.Button = void 0;
...
const date_picker_1 = datePicker;
const date_time_picker_1 = dateTimePicker;
const email_input_1 = emailInput;
const external_multi_select_1 = externalMultiSelect;
const external_select_1 = externalSelect;
const img_1 = img;
const number_input_1 = numberInput;
...
elements$1.DateTimePicker = DateTimePicker;
/**
 * @param {Object} [params] Parameters passed to the constructor.
 * @param {string} [params.actionId] Sets a string to be an identifier for the source of an action in interaction payloads.
 * @param {string} [params.placeholder] Adds the text in place of the input before selected or interacted with.
...
elements$1.EmailInput = EmailInput;
/**
 * @param {Object} [params] Parameters passed to the constructor.
 * @param {string} [params.actionId] Sets a string to be an identifier for the source of an action in interaction payloads.
 * @param {string} [params.placeholder] Adds the text in place of the input before selected or interacted with.
...
elements$1.NumberInput = NumberInput;
/**
 * @param {Object} [params] Parameters passed to the constructor.
 * @param {string} [params.actionId] Sets a string to be an identifier for the source of an action in interaction payloads.
 *
...
class PaginatorUIComponent {
    ...
    getBlocks() {
        ...
        const unpruned = this.paginator.getTotalPages() > 1
            ...
            : blocksForEach.flat();
        return lib_1$2.Builder.pruneUndefinedFromArray(unpruned);
    }
}
...
class AccordionUIComponent {
    ...
    getBlocks() {
        ...
        return lib_1$1.Builder.pruneUndefinedFromArray(unpruned);
    }
}
...
utilities$1.Utilities = utilities$1.buildBlocks = utilities$1.buildBlock = utilities$1.OptionGroupCollection = utilities$1.OptionCollection = utilities$1.AttachmentCollection = utilities$1.BlockCollection = void 0;
const lib_1 = lib;
...
utilities$1.buildBlock = buildBlock;
/**
 * @description Creates and returns an array of built blocks. Behaves in the same way as all appending methods, such as Surface.blocks().
 */
...
utilities$1.buildBlocks = buildBlocks;
const utilities = {
...
})(cypressRunStatus || (cypressRunStatus = {}));
...
const messageConstructor = ({ headingText, channel, status, customBlocks }) => {
    const messageBuilder = dist.Message({ channel, text: headingText }).blocks(dist.Blocks.Section({
        text: headingText ?? 'Cypress Slack Reporter'
    }), dist.Blocks.Divider(), dist.Blocks.Actions().elements(status !== cypressRunStatus['build:failed']
        ? dist.Elements.Button({
            text: 'Test Report'
        })
            .danger(status === cypressRunStatus['test:failed'])
            .primary(status !== cypressRunStatus['test:failed'])
        : undefined, dist.Elements.Button({
        text: 'Build Logs'
    })
...
const sendViaBot = async (opts, client, customBlocks) => {
    const { status, headingText, channel } = opts;
    return await client.chat
        .postMessage(messageConstructor({
        channel,
        headingText,
        status,
        customBlocks
...
(async () => {
    ...
    const channels = coreExports.getInput('channels');
    const workdir = coreExports.getInput('workdir') || 'cypress';
    coreExports.debug(`Token: ${token}`);
    coreExports.debug(`Channels: ${channels}`);
    coreExports.debug('Initializing slack SDK');
    const slack = new dist$4.WebClient(coreExports.getInput('token'));
    coreExports.debug('Slack SDK initialized successfully');
    coreExports.debug('Checking for screenshots from cypress');
    const getScreenshots = async () => {
        const paths = await globby(require$$0$4.resolve(process.cwd(), workdir), {
            expandDirectories: {
                files: ['*'],
                extensions: ['png']
            }
    ...
    if (screenshots.length <= 0) {
        coreExports.debug('No screenshots found. Exiting!');
        coreExports.setOutput('result', 'No videos or screenshots found!');
        return;
    }
    coreExports.debug(`Found ${screenshots.length} screenshots`);
    coreExports.debug('Sending initial slack message');
    const result = await sendViaBot({ status: cypressRunStatus['test:failed'], channel: channels }, slack, [
        dist.Blocks.Section({
    ...
    if (screenshots.length > 0) {
        coreExports.debug('Uploading screenshots...');
        await Promise.all(screenshots.map(async (screenshot) => {
            coreExports.debug(`Uploading ${screenshot}`);
            await slack.files.upload({
                filename: screenshot,
                file: require$$0$3.createReadStream(`${workdir}/${screenshot}`),
                // eslint-disable-next-line camelcase
                thread_ts: threadID,
                channels: channelId
            });
        }));
        coreExports.debug('...done!');
    }
    coreExports.debug('Updating message to indicate a successful upload');
    coreExports.setOutput('result', 'Bingo bango bongo!');
})().catch(error => {
    console.log(error);
