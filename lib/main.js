...
(async () => {
    ...
    coreExports.debug('Checking for videos and/or screenshots from cypress');
    const getScreenshots = async () => {
        const paths = await globby(workdir, {
            expandDirectories: {
                files: ['*'],
                extensions: ['png']
            }
    ...
    coreExports.debug(`${screenshots.length} screenshots`);
    coreExports.debug('Sending initial slack message');
    const result = await sendViaBot({ status: cypressRunStatus['test:failed'], channel: 'github-test' }, slack, [
        dist.Blocks.Section({
