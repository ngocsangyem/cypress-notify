...
const messageConstructor = ({ channel, customBlocks }) => {
    const messageBuilder = dist.Message({ channel })
        .attachments(dist.Attachment({
        color: '#36a64f',
    }))
...
(async () => {
    ...
    const result = await sendViaBot({ channel: channels }, slack);
